...
We can spice up the image by coloring pixels outside of the set based on how quickly they blow up. When actually generating the fractal, you need to specify some *bailout radius* for the program. No computer program can definitively say whether or not $z$ will blow up (except for certain fractals). Instead, after each iteration it checks how far $z$ is from the origin, if it is farther away than the bailout radius then we can confidently say that $z$ will blow up. Based on the number of iterations it takes for $z$ to reach the bailout radius, we color the point. Generating good looking color gradients can take complicated math, so the details of this are in COLORING.md.
...
The choice of the function $z^2+c$ is arbitary, so why not choose functions that generate something more interesting? I decided to use functions of the form $\sum a_{i,j,k} \cdot c^{i} \cdot z^{j} \cdot \bar{z}^{k}$ where each $a_{i,j,k}$ is a constant and $i$, $j$, and $k$ are 0, 1 or 2 (they cannot all be zero). Choosing good values of $a_{i,j,k}$ is the entire purpose of this project. I chose this format for the functions because choosing 27 parameters strikes a good balance between having a lot of possible fractals while not being too computationally expensive. I hope to continue this project by choosing different formats for the function being iterated.
...
Trying to give a value for how 'cool' a fractal is very objective, but for this project, I am using the surface area of the result. I think that the mandelbrot set is too simple and geometric, I want something far more chaotic. Perhaps in later iterations of this project I will consider a different measure, but for now I will use this. We cannot actually find the surface area of a fractal, but we can approximate it by generating the fractal at a certain resolution and finding the surface area of that.

### Gradient descent (sort of)

My first thought for an optimization method was gradient descent. However, there were some problems with this approach. First of all, there is no good way to find the gradient of the surface area. Since the surface area is approximate, it can only be discrete. So there is a limit to the amount you can approximate the actual gradient. The real nail in the coffin was the structure of the function being optimized. Gradient descent is very good at finding local optima, and there are many, many local optima when you are working with 52 parameters. Gradient descent can be helpful for perfecting fractalsls, but ultimately I needed an algorithm for finding global optima. The details of this are found in TODO CREATE FILE ABOUT THISSSS.

### Particle Swarm Optimization

I chose to use particle swarm optimization to find global optima. PSO is simple, cares very little about the structure of the function it is optimizing, and is relatively fast. PSO uses a large cloud of input parameters to get a more large picture view of the function being optimized. You can read more on how PSO works [here] TODO FIND ACTUAL RESOURCE FOR PSO. My implementation of PSO is in TODO ORGANIZE PSO FILE. 
